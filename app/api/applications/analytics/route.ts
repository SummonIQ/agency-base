...
export async function GET(request: NextRequest) {
  try {
    ...
    const whereClause = {
      ...
      submittedAt: query.timeframe !== 'all' ? { gte: since } : undefined,
    ...
    const [
      ...
      db.applicationSubmission.findMany({
        where: {
          ...whereClause,
          status: {
            in: [ApplicationStatus.OFFER_RECEIVED, ApplicationStatus.OFFER_ACCEPTED, ApplicationStatus.OFFER_REJECTED],
          },
        },
        select: {
          status: true,
    ...
    const responseRate = totalApplications > 0 ? (totalResponses / totalApplications) * 100 : 0;
    ...
    const avgResponseTime = responseData.length > 0 
      ? responseData.reduce((sum, app) => sum + (app.daysToResponse || 0), 0) / responseData.length 
      : 0;

    // Average time to offer
    const avgOfferTime = offerData.length > 0
      ? offerData.reduce((sum, app) => sum + (app.daysToFinalOutcome || 0), 0) / offerData.length
      : 0;
    ...
    const statusBreakdown = statusDistribution.map(item => ({
      ...
      percentage: totalApplications > 0 ? (item._count.status / totalApplications) * 100 : 0,
    }));

    // Timeline trends (group by week)
    const timelineTrends = timelineData.reduce((acc, app) => {
    ...
    if (query.type === 'response-time') {
      const responseTimes = responseData.map(r => r.daysToResponse || 0).filter(d => d > 0);
      ...
      const offerDeliveryTimes = offerData.map(o => o.daysToFinalOutcome || 0).filter(d => d > 0);
      ...
      const responseTimeMetrics = {
        avgFirstResponse: responseTimes.length > 0 
          ? responseTimes.reduce((sum, t) => sum + t, 0) / responseTimes.length 
          : 0,
        avgInterviewScheduling: interviewSchedulingTimes.length > 0
          ? interviewSchedulingTimes.reduce((sum, t) => sum + t, 0) / interviewSchedulingTimes.length
          : 0,
        avgOfferDelivery: offerDeliveryTimes.length > 0
          ? offerDeliveryTimes.reduce((sum, t) => sum + t, 0) / offerDeliveryTimes.length
          : 0,
    ...
    const analytics = {
      ...
      conversionFunnel,
      statusBreakdown,
      timelineTrends: Object.values(timelineTrends).sort((a: any, b: any) => 
