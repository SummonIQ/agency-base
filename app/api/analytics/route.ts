...
const analyticsQuerySchema = z.object({
  ...
  type: z.enum(['overview', 'job-leads', 'resumes', 'job-searches']).optional().default('overview'),
});
...
async function getOverviewAnalytics(userId: string, since: Date): Promise<AnalyticsOverview> {
  try {
    const [jobLeadsAgg, jobFitScores] = await Promise.all([
      db.jobLead.groupBy({
        by: ['status'],
        where: {
          userId,
    ...
    const statusCounts = jobLeadsAgg.reduce((acc, item) => {
      acc[item.status] = item._count.status;
      return acc;
    }, {} as Record<JobLeadStatus, number>);

    const totalJobLeads = Object.values(statusCounts).reduce((sum, count) => sum + count, 0);
    const appliedJobs = statusCounts[JobLeadStatus.APPLIED] || 0;
    const interviewsScheduled = statusCounts[JobLeadStatus.INTERVIEW_SCHEDULED] || 0;
    const offersReceived = (statusCounts[JobLeadStatus.OFFER_MADE] || 0) + (statusCounts[JobLeadStatus.OFFER_ACCEPTED] || 0);
    ...
    return {
      ...
      interviewsScheduled,
      offersReceived,
      applicationRate: totalJobLeads > 0 ? (appliedJobs / totalJobLeads) * 100 : 0,
      interviewRate: appliedJobs > 0 ? (interviewsScheduled / appliedJobs) * 100 : 0,
      offerRate: interviewsScheduled > 0 ? (offersReceived / interviewsScheduled) * 100 : 0,
...
async function getJobLeadAnalytics(userId: string, since: Date): Promise<JobLeadAnalytics> {
  const [statusDistribution, topCompanies, locationDistribution] = await Promise.all([
    db.jobLead.groupBy({
      by: ['status'],
      where: {
        userId,
    ...
    db.$queryRaw<Array<{company: string; count: bigint; avgFitScore: number}>>
      `SELECT 
        jl.company,
        COUNT(*) as count,
      ...
      ORDER BY count DESC
      LIMIT 10`,
    db.$queryRaw<Array<{location: string; count: bigint}>>
      `SELECT 
        jl.location,
        COUNT(*) as count
      FROM "JobListing" jl
  ...
  return {
    ...
    statusProgression: [], // TODO: Implement time-series progression
...
async function getResumeAnalytics(userId: string, since: Date): Promise<ResumeAnalytics> {
  ...
  const avgScore = optimizations.length > 0 
    ? optimizations.reduce((sum, opt) => sum + (opt.score || 0), 0) / optimizations.length
    : 0;
...
async function getJobSearchAnalytics(userId: string, since: Date): Promise<JobSearchAnalytics> {
  const [searches, searchTerms] = await Promise.all([
    ...
    db.$queryRaw<Array<{searchTerm: string; count: bigint; avgResults: number}>>
      `SELECT 
        "searchTerm",
        COUNT(*) as count,
  ...
  const totalJobs = searches.reduce((sum, search) => sum + search._count.jobListings, 0);
  ...
  return {
    ...
    searchSuccessRate: searches.length > 0 ? (completedSearches / searches.length) * 100 : 0,
  };
}
