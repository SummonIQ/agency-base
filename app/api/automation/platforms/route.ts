...
function handlePlatformCapabilities(platform: string | null) {
  ...
  const jobBoard = platform.toUpperCase() as JobBoard;
  if (!Object.values(JobBoard).includes(jobBoard)) {
    return NextResponse.json(
  ...
  const capabilities = getPlatformCapabilities(jobBoard);
  return NextResponse.json({ platform: jobBoard, capabilities });
}
...
function handleFieldMapping(platform: string | null) {
  ...
  const mapping = getFieldMapping(jobBoard);
...
function handlePlatformList() {
  const platforms = Object.values(JobBoard).map(platform => {
...
function handleFormValidation(platform: string, formData: Record<string, any>) {
  ...
  const validation = validateFormData(jobBoard, formData);
  return NextResponse.json({
    platform: jobBoard,
    validation,
  });
