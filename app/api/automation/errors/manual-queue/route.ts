  ...
  try {
    ...
    const queueItems = [
      ...
      ...manualReviewApps.map(app => ({
        id: app.id,
        jobTitle: app.jobLead?.jobTitle || 'Unknown Job',
        company: app.jobLead?.companyName || 'Unknown Company',
        platform: app.jobLead?.platform || 'Unknown',
        errorMessage: (app.metadata as any)?.manualReviewReason || 'Requires manual review',
