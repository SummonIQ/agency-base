  ...
  try {
    ...
    if (errorLog) {
      ...
      if (context.applicationId || context.jobLeadId) {
        await db.automationAuditLog.deleteMany({
          where: {
            ...
            metadata: {
              path: ['context', context.applicationId ? 'applicationId' : 'jobLeadId'],
      ...
      await createNotification({
        userId: user.id,
        type: 'automation_error_resolved',
        title: 'Error Resolved',
        content: `Error for ${metadata.context?.metadata?.jobTitle || 'application'} has been resolved`,
        metadata: {
          errorId,
          resolutionMethod,
        },
