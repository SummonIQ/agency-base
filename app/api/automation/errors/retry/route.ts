  ...
  try {
    ...
    if (applicationId) {
      ...
      if (immediate) {
        ...
        // Use intelligent scheduler to find optimal retry time
        const scheduler = IntelligentScheduler.getInstance();
        const optimalTime = await scheduler.findOptimalTime(
          scheduledApp.jobLead.platform || 'generic',
          user.id
        );
        retryAt = optimalTime || addMinutes(new Date(), 15); // Default to 15 minutes
      }
      ...
      await db.automationAuditLog.create({
        data: {
          ...
          metadata: {
            ...
            immediate,
            jobTitle: scheduledApp.jobLead.jobTitle,
            company: scheduledApp.jobLead.companyName,
          },
        },
      });
      ...
      return NextResponse.json({
        ...
        applicationId,
        retryAt: retryAt.toISOString(),
        message: immediate ? 'Application scheduled for immediate retry' : `Application scheduled for retry at ${retryAt.toLocaleString()}`,
      });
    }
    ...
    if (errorId) {
      ...
      if (context.jobLeadId) {
        ...
        // Create new scheduled application
        const scheduler = IntelligentScheduler.getInstance();
        const optimalTime = await scheduler.findOptimalTime(
          jobLead.platform || 'generic',
          user.id
        );
        ...
        const newScheduledApp = await db.automationScheduledApplication.create({
          data: {
            ...
            status: 'scheduled',
            scheduledFor: optimalTime || addMinutes(new Date(), 15),
            attemptCount: 1,
            metadata: {
