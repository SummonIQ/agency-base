...
export async function GET(request: Request) {
  try {
    ...
    csvRows.push(['Average Processing Time:', `${metrics.averageProcessingTime.toFixed(2)} minutes`]);
    csvRows.push(['Time Saved:', `${metrics.roiMetrics.totalTimeSaved.toFixed(2)} hours`]);
    csvRows.push([]);
    
    // Platform Breakdown
    csvRows.push(['Platform Performance']);
    csvRows.push(['Platform', 'Total Submissions', 'Success Count', 'Failure Count', 'Success Rate', 'Avg Time (min)']);
    ...
    csvRows.push(['Job Title', 'Company', 'Platform', 'Status', 'Submitted At', 'Processing Time (min)']);
    metrics.recentActivity.forEach(activity => {
      csvRows.push([
        ...
        activity.platform,
        activity.status,
        activity.submittedAt ? format(new Date(activity.submittedAt), 'yyyy-MM-dd HH:mm:ss') : 'N/A',
