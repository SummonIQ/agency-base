...
const handleGET = async (request: NextRequest): Promise<NextResponse> => {
  ...
  const resourceType = url.searchParams.get('resourceType') as ShareableResourceType;
  const feedbackOnly = url.searchParams.get('feedbackOnly') === 'true';
  
  if (!resourceId || !resourceType) {
    return NextResponse.json(
      { success: false, message: 'resourceId and resourceType are required query parameters' },
      { status: 400 }
    );
  }
