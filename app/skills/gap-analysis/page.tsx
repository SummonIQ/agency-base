...
export default function SkillGapAnalysisPage() {
  ...
  const [selectedJobLeadId, setSelectedJobLeadId] = useState(searchParams.get('jobLeadId') || '');
        ...
        if (jobLeadsRes.ok && resumesRes.ok && analysesRes.ok) {
          ...
          if (searchParams.get('jobLeadId')) {
            const existingAnalysis = analysesData.data?.find(
              (a: SkillGapAnalysis) => a.jobLeadId === searchParams.get('jobLeadId')
            );
  ...
  return (
    <div className="container mx-auto py-10 space-y-8">
      <div className="flex flex-col">
        ...
        <p className="text-muted-foreground">Analyze the gap between your skills and job requirements</p>
      </div>
          ...
          <Card>
            ...
            <CardFooter>
              <Button 
                ...
                disabled={isLoading || isAnalyzing || !selectedJobLeadId || !selectedResumeId}
                className="w-full"
              >
              ...
              {isLoading ? (
                ...
                <div className="space-y-2">
                  {savedAnalyses.map((item) => {
                    ...
                    const job = jobLeads.find((j: any) => j.id === item.jobLeadId);
                    const resume = resumes.find((r: any) => r.id === item.resumeId);
                ...
                <Tabs defaultValue="recommendations">
                  ...
                  <TabsContent value="recommendations" className="space-y-4 mt-4">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="space-y-2 p-4 border rounded-lg bg-gray-50">
                        ...
                        <ul className="list-disc pl-5 space-y-1">
                          {analysis.recommendations.skillsToAcquire.map((skill, index) => (
                      ...
                      <div className="space-y-2 p-4 border rounded-lg bg-gray-50">
                        ...
                        <ul className="list-disc pl-5 space-y-1">
                          {analysis.recommendations.skillsToImprove.map((skill, index) => (
                            <li key={index} className="text-sm">{skill}</li>
                          ))}
                        </ul>
                      </div>
                    ...
                    <div className="space-y-3">
                      ...
                      {analysis.recommendations.coursesAndResources.map((item, index) => (
                        <div key={index} className="p-3 border rounded-md">
                          <h5 className="font-medium text-primary">{item.skill}</h5>
                              ...
                              <div key={i} className="text-sm">
                                ...
                                {resource.estimatedTimeHours && (
                                  <p className="text-xs text-gray-500 mt-1">
                                    Estimated time: {resource.estimatedTimeHours} hours
                                  </p>
                                )}
                              </div>
                            ))}
                          ...
                          <div className="flex justify-between items-start">
                            ...
                            <span 
                              className={`text-xs px-2 py-1 rounded-full ${
                                skill.type === 'technical' ? 'bg-blue-100 text-blue-800' :
                                skill.type === 'soft' ? 'bg-green-100 text-green-800' :
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                      ...
                      {analysis.partialSkills.length > 0 && (
                        <div className="col-span-full mt-4">
                          <h3 className="font-semibold mb-2">Partial Matches</h3>
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            ...
            <Card className="h-full flex items-center justify-center">
              <CardContent className="py-12 text-center">
                ...
                <p className="text-muted-foreground">
                  Select a job and resume to analyze the skill gap, or choose a saved analysis
                </p>
              </CardContent>
            </Card>
          )}
