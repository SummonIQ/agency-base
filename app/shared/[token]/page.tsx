    ...
    const fetchResource = async () => {
      try {
        ...
        let transformedResource: SharedResource = {
          id: resource.id,
          type: resource.jobListing ? ShareableResourceType.JOB_LEAD : ShareableResourceType.RESUME,
          name: resource.jobListing?.title || resource.name || 'Shared Resource',
          content: resource,
        ...
        toast({
          title: 'Error',
          description: 'Failed to load the shared resource. The link may be expired or invalid.',
          variant: 'destructive',
        });
      } finally {
        setIsLoading(false);
    ...
    return (
      <div className="flex h-screen items-center justify-center">
        <div className="text-center">
          <Spinner size="lg" />
          <p className="mt-4 text-muted-foreground">Loading shared resource...</p>
        </div>
      </div>
    );
  }
  ...
  if (!resource) {
    return (
      <div className="container mx-auto max-w-2xl py-16">
        <Card>
          <CardHeader>
            <CardTitle className="text-red-500">Error Loading Resource</CardTitle>
            <CardDescription>
              The shared resource could not be loaded. The link may be expired or invalid.
            </CardDescription>
          </CardHeader>
          <CardContent>
    ...
    <div className="container mx-auto max-w-5xl py-8">
      <div className="mb-6">
        <div className="flex items-center">
          ...
          <span className="ml-3 px-2 py-1 text-xs font-medium bg-blue-100 text-blue-800 rounded">
            Shared {resource.type === ShareableResourceType.JOB_LEAD ? 'Job Lead' : 'Resume'}
          </span>
        </div>
      </div>
      ...
      <Tabs defaultValue="resource" className="space-y-4">
        ...
        <TabsContent value="resource" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle>{resource.type === ShareableResourceType.JOB_LEAD ? 'Job Lead Details' : 'Resume'}</CardTitle>
            </CardHeader>
            <CardContent>
              {resource.type === ShareableResourceType.JOB_LEAD ? (
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <p>{resource.content.jobListing?.company || 'Not specified'}</p>
                  </div>
                  <div>
                    <h3 className="font-medium">Location</h3>
                    <p>{resource.content.jobListing?.location || 'Not specified'}</p>
                  </div>
                  <div>
                    <h3 className="font-medium">Description</h3>
                    <div className="whitespace-pre-wrap rounded-md border p-4 bg-gray-50">
                      {resource.content.jobListing?.description || 'No description provided'}
                    </div>
                  </div>
                </div>
              ) : (
                ...
                <div className="space-y-4">
                  {resource.content.markdown ? (
                    ...
                    <p>This resume has no content available in markdown format.</p>
                  )}
                </div>
              )}
            </CardContent>
          ...
          <TabsContent value="feedback">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Share your thoughts and suggestions about this {resource.type === ShareableResourceType.JOB_LEAD ? 'job lead' : 'resume'}
                </CardDescription>
              </CardHeader>
              <CardContent>
                {!feedbackSent ? (
