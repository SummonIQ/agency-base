...
import { InterviewType, DifficultyLevel, InterviewQuestion } from '@/lib/interviews/types';
...
function InterviewSimulateContent() {
  ...
  const [jobLeadId, setJobLeadId] = useState(searchParams.get('jobLeadId') || '');
  ...
  const generateOverallFeedback = (allResponses: any[]) => {
    ...
    if (avg >= 85) {
      ...
      setOverallFeedback(
        ...
        "Review the detailed feedback for each question and practice refining those answers before your actual interviews."
      );
    } else {
      setOverallFeedback(
  ...
  const progress = questions.length > 0 
    ? ((currentQuestionIndex + (simulationComplete ? 1 : 0)) / questions.length) * 100
    : 0;
  
  // Get current question and response
  const currentQuestion = questions[currentQuestionIndex];
  const currentResponse = responses.find(r => 
    currentQuestion && r.questionId === currentQuestion.id
  );
  ...
  return (
    <div className="container mx-auto py-10 space-y-8">
      <div className="flex flex-col">
        ...
        <p className="text-muted-foreground">Practice and get AI feedback on your interview responses</p>
      </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
              ...
              <div className="space-y-2 md:col-span-2">
                <label className="flex items-center space-x-2">
                  <input
                    ...
                    disabled={isGenerating}
                  />
                  <span className="text-sm font-medium">Show timer during questions</span>
                </label>
              </div>
            </div>
            ...
            >
              ...
              {isGenerating ? 'Generating Questions...' : 'Start Interview Simulation'}
            </Button>
          </CardContent>
        </Card>
      ) : (
              ...
              <CardContent className="space-y-6">
                <div className="flex items-center justify-center p-6">
                  <div className="text-center">
                    <h3 className="text-2xl font-bold mb-2">Your Overall Score</h3>
                    <div className="text-5xl font-bold text-primary">{averageScore}/100</div>
                  </div>
                </div>
                      ...
                      <div>
                        ...
                        <div className="space-y-3">
                          {responses.map((response, index) => {
                            ...
                            return (
                              <div key={response.questionId} className="flex items-center">
