...
import { Calendar, Clock, TrendingUp, AlertCircle, CheckCircle, XCircle, RefreshCw, BarChart3, Target } from 'lucide-react';
...
export default function SchedulingDashboard() {
  ...
  const [selectedView, setSelectedView] = useState<'timeline' | 'calendar' | 'analytics'>('timeline');
  ...
  const handleReschedule = async (applicationId: string) => {
    try {
      const res = await fetch(`/api/automation/scheduled/${applicationId}/reschedule`, {
        method: 'POST',
      });
        ...
        <Card key={app.id} className="relative">
          {index === 0 && (
            <div className="absolute -top-2 -left-2">
              <Badge variant="default" className="bg-blue-600">Next Up</Badge>
            </div>
          )}
              ...
              <div className="flex-1 space-y-2">
                ...
                <div className="flex items-center gap-4 text-sm">
                  <div className="flex items-center gap-1">
                    ...
                    <span>{formatDateTimeInTimezone(app.scheduledFor, userTimezone, {
                      month: 'short',
                      day: 'numeric',
                      hour: '2-digit',
                      minute: '2-digit',
                  ...
                  {app.optimalityScore !== undefined && (
                    <div className="flex items-center gap-1">
                      ...
                      <span className={cn('font-medium', getOptimalityColor(app.optimalityScore))}>
                        {getOptimalityLabel(app.optimalityScore)} ({app.optimalityScore}%)
                      </span>
                    </div>
                  )}
                </div>
      ...
      {scheduledApplications.length === 0 && (
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <Button className="mt-4" onClick={() => window.location.href = '/tools/automation'}>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{analytics.todayScheduled}</div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{analytics.weekScheduled}</div>
            </CardContent>
          </Card>
          
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium">Avg. Optimality</CardTitle>
            </CardHeader>
            <CardContent>
              <div className={cn('text-2xl font-bold', getOptimalityColor(analytics.averageOptimalityScore))}>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Applications scheduled by platform</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Applications scheduled by hour of day</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Best times for scheduling new applications</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-2">
              {analytics.upcomingOptimalSlots.map((slot, index) => (
                <div key={index} className="flex items-center justify-between p-2 rounded hover:bg-gray-50">
                  <div className="flex items-center gap-2">
                    <TrendingUp className={cn('h-4 w-4', getOptimalityColor(slot.score))} />
                  ...
                  <Badge variant="outline" className={cn(getOptimalityColor(slot.score))}>
                    {slot.score}% optimal
                  </Badge>
                </div>
              ))}
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold">Intelligent Scheduling Dashboard</h1>
          <p className="text-muted-foreground mt-1">
            Optimize your application timing for maximum visibility
          </p>
        </div>
        <Button onClick={fetchSchedulingData} disabled={loading}>
