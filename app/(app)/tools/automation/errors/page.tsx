                        ...
                        <div className="flex-1 space-y-2">
                          <div className="flex items-center gap-2">
                            ...
                            <h3 className="font-semibold">
                              {error.jobTitle || 'Unknown Job'} at {error.company || 'Unknown Company'}
                            </h3>
                            ...
                            {error.platform && (
                              <Badge variant="secondary">{error.platform}</Badge>
                            )}
                          </div>
                          ...
                          <Dialog>
                            ...
                            <DialogContent className="max-w-2xl">
                              <DialogHeader>
                                ...
                                <DialogDescription>
                                  Full error information and resolution options
                                </DialogDescription>
                              </DialogHeader>
                              <div className="space-y-4">
                                <div>
                                ...
                                <div>
                                  ...
                                  <div className="text-sm space-y-1">
                                    ...
                                    <p>Timestamp: {format(new Date(error.timestamp), 'PPpp')}</p>
                                  </div>
                                </div>
                                {error.suggestedAction && (
                                  <div>
                                ...
                                <div className="flex gap-2">
                                  <Button
                                    onClick={() => handleResolve(error.id, 'manual')}
                                    variant="outline"
                                  >
                                    Mark as Resolved
                                  </Button>
                                  {error.requiresUserAction && (
                                    <Button
                                      onClick={() => window.location.href = `/tools/automation/fix/${error.id}`}
                                    >
                                      Fix Manually
                                    </Button>
                                  )}
        ...
        <TabsContent value="manual" className="space-y-4">
          {manualQueue.length === 0 ? (
            <Card>
              <CardContent className="p-12 text-center">
                <CheckCircle className="h-12 w-12 mx-auto text-green-500 mb-4" />
                <p className="text-muted-foreground">No items requiring manual intervention</p>
              </CardContent>
            </Card>
          ) : (
            <div className="space-y-4">
                ...
                <Card key={item.id}>
                  <CardContent className="p-6">
                    <div className="flex items-start justify-between">
                      ...
                      <Button
                        onClick={() => window.location.href = `/tools/automation/manual/${item.id}`}
                      >
                        Review & Fix
                      </Button>
                    </div>
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-medium">{error.jobTitle}</span>
                          <Badge variant="outline">{error.category}</Badge>
                        </div>
                        <div className="text-sm text-muted-foreground">
                          Resolved {error.resolutionMethod && `via ${error.resolutionMethod}`}
                          {' '}
                          {error.resolvedAt && formatDistanceToNow(new Date(error.resolvedAt), { addSuffix: true })}
                        </div>
                      </div>
                    </CardContent>
                  </Card>
