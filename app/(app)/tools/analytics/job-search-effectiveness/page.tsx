...
export default function JobSearchEffectivenessPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/analytics/job-search-effectiveness?type=overview&period=${period}`);
  ...
  const getRecommendationColor = (recommendation: string) => {
    switch (recommendation) {
      ...
      case 'suboptimal': return 'text-orange-600 bg-orange-100';
      default: return 'text-gray-600 bg-gray-100';
    }
  };
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">Analyzing job search effectiveness...</p>
        </div>
      </div>
    );
  }
  ...
  if (!data) {
    return (
      <Alert>
        ...
        <AlertDescription>
          We need more job search and application data to generate effectiveness insights.
        </AlertDescription>
      </Alert>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Job Search Effectiveness</h1>
          <p className="text-muted-foreground mt-2">
            Analyze and optimize your job search strategy with data-driven insights
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            ...
            <div className="flex items-center mt-2">
              <Badge className={getEffectivenessColor(data.strategies[0]?.effectiveness || 'low')}>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium">Optimal Time Slots</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium">Top Platform ROI</CardTitle>
          </CardHeader>
          <CardContent>
            ...
            <Card>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <PieChart>
                    <Pie
                      data={data.platforms.slice(0, 6)}
                      cx="50%"
                      cy="50%"
                      outerRadius={100}
                      fill="#8884d8"
                      dataKey="roi"
                      label={({ platform, roi }) => `${platform}: ${roi.toFixed(1)}`}
                    >
                  ...
                  {data.keywords.slice(0, 6).map((keyword, index) => (
                    <div key={keyword.keyword} className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        ...
                        <Badge className={cn(
                          "text-xs",
                          keyword.optimization === 'expand' ? 'bg-green-100 text-green-800' :
                          keyword.optimization === 'maintain' ? 'bg-blue-100 text-blue-800' :
                          keyword.optimization === 'refine' ? 'bg-yellow-100 text-yellow-800' :
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <p className="font-medium">{strategy.totalApplications}</p>
                    </div>
                    <div>
                      <p className="text-muted-foreground">Success Rate</p>
                      <p className="font-medium text-green-600">{strategy.successRate.toFixed(1)}%</p>
                    </div>
                  </div>
                    ...
                    <div className="space-y-2">
                      {strategy.platforms.slice(0, 3).map((platform) => (
                        <div key={platform.platform} className="flex items-center justify-between text-sm">
                          ...
                          <span className="text-muted-foreground">{platform.usage} jobs</span>
                        </div>
                      ))}
                    </div>
                  </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
            {data.timing.map((timing) => (
              <Card key={timing.timeSlot}>
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <CardTitle className="text-lg">{timing.timeSlot}</CardTitle>
                    <Badge className={getRecommendationColor(timing.recommendation)}>
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      <p className="text-muted-foreground">Applications</p>
                      <p className="font-medium">{timing.totalApplications}</p>
                    </div>
                    <div>
                      <p className="text-muted-foreground">Success Rate</p>
                      <p className="font-medium text-green-600">{timing.successRate.toFixed(1)}%</p>
                    </div>
                    <div>
                      <p className="text-muted-foreground">Successful</p>
                      <p className="font-medium">{timing.successfulApplications}</p>
                    </div>
                    <div>
                      <p className="text-muted-foreground">Avg Response</p>
                      <p className="font-medium">{timing.averageResponseTime.toFixed(1)} days</p>
                    </div>
                  </div>
                  ...
                  {timing.recommendation === 'optimal' && (
                    <Alert>
                      ...
                      <AlertDescription className="text-sm">
                        Optimal time slot! Consider scheduling more applications during this period.
                      </AlertDescription>
                    </Alert>
                  )}
                </CardContent>
              ...
              <Card key={platform.platform}>
                <CardHeader>
                  <div className="flex items-center justify-between">
                    ...
                    <Badge className={cn(
                      platform.costEffectiveness === 'excellent' ? 'bg-green-100 text-green-800' :
                      platform.costEffectiveness === 'good' ? 'bg-blue-100 text-blue-800' :
                      platform.costEffectiveness === 'fair' ? 'bg-yellow-100 text-yellow-800' :
                      'bg-red-100 text-red-800'
                    )}>
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                      <p className="text-muted-foreground">Applications</p>
                      <p className="font-medium">{platform.totalApplications}</p>
                    </div>
                    <div>
                      <p className="text-muted-foreground">Success Rate</p>
                    ...
                    <div>
                      ...
                      <p className="font-medium">{platform.averageTimeInvested.toFixed(1)}h</p>
                    </div>
                    <div>
                      <p className="text-muted-foreground">ROI</p>
                      <p className="font-medium text-purple-600">{platform.roi.toFixed(1)}</p>
                    </div>
                  </div>
            ...
            {data.keywords.map((keyword) => (
              <Card key={keyword.keyword}>
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      <p className="text-muted-foreground">Search Frequency</p>
                    ...
                    <div>
                      ...
                      <p className="font-medium">{keyword.relevanceScore.toFixed(1)}%</p>
                    </div>
                    <div>
                      <p className="text-muted-foreground">Success Rate</p>
                      <p className="font-medium text-green-600">{keyword.successRate.toFixed(1)}%</p>
                    </div>
                  </div>

                  <div>
                    <p className="text-sm font-medium mb-2">Relevance Score</p>
                    <Progress value={keyword.relevanceScore} className="h-2" />
                  </div>
                  ...
                  {keyword.optimization === 'expand' && (
                    <Alert>
                      ...
                      <AlertDescription className="text-sm">
                        High-performing keyword! Consider using in more searches.
                      </AlertDescription>
                    </Alert>
                  )}
                </CardContent>
