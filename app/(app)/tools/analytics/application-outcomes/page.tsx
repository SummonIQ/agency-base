  ...
  const fetchData = async () => {
    ...
    try {
      const [metricsRes, responseRes] = await Promise.all([
        ...
        fetch(`/api/applications/analytics?type=response-time&timeframe=${timeframe}`),
      ]);
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">Loading application outcome data...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Application Outcome Tracking</h1>
          <p className="text-muted-foreground mt-2">
            Track success rates, response times, and conversion funnels for your job applications
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-2">
              {metrics.timeSeriesData.length > 1 && (
                <>
                  {metrics.timeSeriesData[metrics.timeSeriesData.length - 1].applications > 
                   metrics.timeSeriesData[metrics.timeSeriesData.length - 2].applications ? (
        ...
        <Card>
          ...
          <CardContent>
            <div className={cn("text-2xl font-bold", getSuccessRateColor(metrics.successRate).split(' ')[0])}>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-2">
              {responseMetrics && `Avg ${formatDays(responseMetrics.avgFirstResponse)}`}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-2">
              {Math.round(metrics.totalApplications * metrics.interviewRate / 100)} interviews
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-2">
              {Math.round(metrics.totalApplications * metrics.offerRate / 100)} offers
            </p>
          </CardContent>
        </Card>
      </div>
                ...
                <ResponsiveContainer width="100%" height={300}>
                  ...
                  >
                    ...
                    <Bar dataKey="rate" fill="#3b82f6">
                      <LabelList dataKey="rate" position="top" formatter={(value: any) => `${value.toFixed(1)}%`} />
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </CardContent>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <Badge variant="outline">{metrics.performanceByAutomation.automated.total}</Badge>
                      </div>
                      <Progress 
                        value={metrics.performanceByAutomation.automated.responseRate} 
                        className="h-2"
                      />
                      <p className="text-xs text-muted-foreground">
                        {metrics.performanceByAutomation.automated.responseRate.toFixed(1)}% response rate
                      </p>
                      <p className="text-xs font-medium text-green-600">
                        {metrics.performanceByAutomation.automated.successRate.toFixed(1)}% success rate
                      </p>
                    </div>
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-medium">Manual</span>
                        <Badge variant="outline">{metrics.performanceByAutomation.manual.total}</Badge>
                      </div>
                      <Progress 
                        value={metrics.performanceByAutomation.manual.responseRate} 
                        className="h-2"
                      />
                      <p className="text-xs text-muted-foreground">
                        {metrics.performanceByAutomation.manual.responseRate.toFixed(1)}% response rate
                      </p>
                      <p className="text-xs font-medium text-green-600">
                        {metrics.performanceByAutomation.manual.successRate.toFixed(1)}% success rate
                      </p>
                    </div>
                  </div>
                  ...
                  <ResponsiveContainer width="100%" height={200}>
                    <BarChart 
                      data={[
                        { 
                          type: 'Automated', 
                          responseRate: metrics.performanceByAutomation.automated.responseRate,
                          successRate: metrics.performanceByAutomation.automated.successRate,
                        },
                        { 
                          type: 'Manual', 
                          responseRate: metrics.performanceByAutomation.manual.responseRate,
                          successRate: metrics.performanceByAutomation.manual.successRate,
                        },
                      ]}
                    >
                      <CartesianGrid strokeDasharray="3 3" />
                ...
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart 
                    data={Object.entries(metrics.statusBreakdown).map(([status, count]) => ({
                      ...
                      count,
                      percentage: ((count / metrics.totalApplications) * 100).toFixed(1),
                    }))}
                  >
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis 
        ...
        <TabsContent value="funnel" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Track progression through each stage of the application process
              </CardDescription>
            </CardHeader>
            <CardContent>
                      ...
                      content={(props: any) => {
                        ...
                        return (
                          <text 
                            ...
                            fontWeight="bold"
                          >
                            <tspan x={props.x} dy="-10">{payload.stage}</tspan>
                            <tspan x={props.x} dy="20">{value} ({payload.percentage.toFixed(1)}%)</tspan>
              ...
              <div className="mt-6 space-y-2">
                {metrics.conversionFunnel.map((step, index) => (
                  <div key={step.stage} className="flex items-center justify-between py-2 border-b last:border-0">
                      ...
                      <div>
                        ...
                        {step.averageDaysToReach && (
                          <p className="text-sm text-muted-foreground">
                            Avg. time to reach: {formatDays(step.averageDaysToReach)}
                          </p>
                        )}
                      </div>
                    </div>
                    <div className="text-right">
                      ...
                      <p className="text-sm text-muted-foreground">{step.percentage.toFixed(1)}% of total</p>
                    </div>
                  </div>
                ))}
              </div>
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="text-2xl font-bold">{formatDays(responseMetrics.avgFirstResponse)}</p>
                      </div>
                      <div>
                        <p className="text-sm text-muted-foreground">Interview Scheduling</p>
                        <p className="text-2xl font-bold">{formatDays(responseMetrics.avgInterviewScheduling)}</p>
                      </div>
                      <div>
                        <p className="text-sm text-muted-foreground">Offer Delivery</p>
                        <p className="text-2xl font-bold">{formatDays(responseMetrics.avgOfferDelivery)}</p>
                      </div>
                      <div>
                        <p className="text-sm text-muted-foreground">Range</p>
                        <p className="text-lg font-bold">
                          {formatDays(responseMetrics.fastestResponse)} - {formatDays(responseMetrics.slowestResponse)}
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                {responseMetrics && (
                  <ResponsiveContainer width="100%" height={250}>
                    <BarChart data={responseMetrics.responseTimeDistribution}>
                          ...
                          if (active && payload && payload[0]) {
                            return (
                              <div className="bg-background border rounded p-2">
                                ...
                                <p className="text-sm">{payload[0].value} applications</p>
                                <p className="text-sm">{payload[0].payload.percentage}% of responses</p>
                              </div>
                            );
                          }
                          return null;
                      ...
                      />
                      <Bar dataKey="count" fill="#3b82f6">
                        <LabelList dataKey="percentage" position="top" formatter={(value: any) => `${value}%`} />
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                )}
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="text-center">
                  ...
                  <p className="text-2xl font-bold">{formatDays(metrics.averageTimeToFinalOutcome)}</p>
                </div>
                <div className="text-center">
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <PieChart>
                    <Pie
                      data={[
                        { name: 'Offers Accepted', value: metrics.statusBreakdown.OFFER_ACCEPTED || 0 },
                        { name: 'Offers Received', value: metrics.statusBreakdown.OFFER_RECEIVED || 0 },
                        { name: 'Rejected', value: metrics.statusBreakdown.REJECTED || 0 },
                        { name: 'Not Selected', value: metrics.statusBreakdown.NOT_SELECTED || 0 },
                        { name: 'Withdrawn', value: metrics.statusBreakdown.WITHDRAWN || 0 },
                  ...
                  <div key={platform.platform} className="flex items-center justify-between py-2 border-b last:border-0">
                    ...
                    <div className="flex items-center gap-4">
                      <div className="text-right">
                        ...
                        <p className="font-medium">{platform.responseRate.toFixed(1)}%</p>
                      </div>
                      <div className="text-right">
                        <p className="text-sm text-muted-foreground">Interview</p>
                        <p className="font-medium">{platform.interviewRate.toFixed(1)}%</p>
                      </div>
                      <div className="text-right">
                        <p className="text-sm text-muted-foreground">Success</p>
                        <p className="font-medium text-green-600">{platform.successRate.toFixed(1)}%</p>
                      </div>
                    </div>
                  </div>
                ))}
