...
import { 
  ...
  ResponseRatesChart, 
  JobBoardPerformanceChart,
  ResumePerformanceChart,
  StatCard
} from '@/components/analytics/charts';
import { createAnalyticsPeriod, getUserAnalytics, AnalyticsData } from '@/lib/analytics';
...
export default function AnalyticsDashboard() {
  ...
  useEffect(() => {
    async function fetchAnalytics() {
      setLoading(true);
      try {
        const data = await getUserAnalytics(createAnalyticsPeriod(Number(period)));
  ...
  const statusDistributionData = analyticsData ? [
  ...
  const responseRatesData = analyticsData ? [
    { name: 'Responses', value: analyticsData.responseRates.responseRate },
    { name: 'Interviews', value: analyticsData.responseRates.interviewRate },
      ...
      <Tabs defaultValue="overview" className="space-y-8">
            ...
            {loading ? (
              ...
              <>
                <StatCard
                  ...
                  value={analyticsData?.timeToResponse.averageDaysToFirstResponse?.toFixed(1) || 'N/A'}
                  description="From application submission to first response"
                />
                <StatCard
                  title="Avg. Days to Interview"
                  value={analyticsData?.timeToResponse.averageDaysToInterview?.toFixed(1) || 'N/A'}
                  description="From application submission to first interview"
                />
                <StatCard
                  title="Avg. Days to Offer"
                  value={analyticsData?.timeToResponse.averageDaysToOffer?.toFixed(1) || 'N/A'}
        ...
        <TabsContent value="job-boards" className="space-y-6">
          {loading ? (
            ...
            <JobBoardPerformanceChart data={analyticsData?.jobBoardPerformance || []} />
          )}
                ...
                <table className="w-full table-auto">
                  ...
                  <tbody>
                    {loading ? (
                      ...
                      analyticsData?.jobBoardPerformance.map((board) => (
                        <tr key={board.jobBoard} className="border-b">
                          <td className="py-3 px-4">{board.jobBoard}</td>
                          <td className="py-3 px-4 text-right">{board.applications}</td>
                          <td className="py-3 px-4 text-right">{formatPercentage(board.responseRate / 100)}</td>
                          <td className="py-3 px-4 text-right">{formatPercentage(board.interviewRate / 100)}</td>
                          <td className="py-3 px-4 text-right">{formatPercentage(board.offerRate / 100)}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
        ...
        <TabsContent value="resumes" className="space-y-6">
          {loading ? (
            ...
            <ResumePerformanceChart data={analyticsData?.resumePerformance || []} />
          )}
                    ...
                    {loading ? (
                      ...
                      analyticsData?.resumePerformance.map((resume) => (
                        <tr key={resume.resumeId} className="border-b">
                          ...
                          <td className="py-3 px-4 text-right">{resume.applications}</td>
                          <td className="py-3 px-4 text-right">{resume.responses}</td>
                          <td className="py-3 px-4 text-right">{resume.interviews}</td>
                          <td className="py-3 px-4 text-right">{resume.offers}</td>
                          <td className="py-3 px-4 text-right">{formatPercentage(resume.responseRate / 100)}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
          ...
          <Card>
            ...
            <CardContent>
              <p className="text-muted-foreground mb-4">
                Trend analysis will be implemented in a future update. This section will show application volume and outcomes over time.
              </p>
