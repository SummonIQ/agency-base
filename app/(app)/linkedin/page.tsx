...
export const metadata: Metadata = {
  title: "Professional Profile | AgencyBase",
  description: "Manage your professional profile integration",
};
...
export default async function LinkedInPage() {
  ...
  return (
    <Page>
      <PageHeader>
        <PageSummary>
          <PageTitle>LinkedIn Profile</PageTitle>
          <PageDescription>
            Manage your LinkedIn profile integration and networking
          </PageDescription>
        </PageSummary>
      </PageHeader>
                      ...
                      <Tabs defaultValue="experience" className="w-full">
                        ...
                        <TabsContent value="experience" className="space-y-4 mt-4">
                          {profileData?.positions && profileData.positions.length > 0 ? (
                            <ul className="space-y-4">
                              {profileData.positions.map((position: any, index: number) => (
                                <li key={index} className="border-b pb-3 last:border-b-0 last:pb-0">
                                  ...
                                  {position.description && (
                                    <div className="text-sm mt-2">{position.description}</div>
                                  )}
                                </li>
                              ))}
                            </ul>
                        ...
                        <TabsContent value="education" className="space-y-4 mt-4">
                          {profileData?.education && profileData.education.length > 0 ? (
                                ...
                                <li key={index} className="border-b pb-3 last:border-b-0 last:pb-0">
                                  ...
                                  {education.startDate && (
                                    <div className="text-sm text-muted-foreground">
                                      ...
                                      {education.endDate ? ` to ${education.endDate.year}` : " to Present"}
                                    </div>
                                  )}
                            ...
                            <div className="text-center py-8 text-muted-foreground">
                              No education information found in your LinkedIn profile.
                            </div>
                          )}
                        </TabsContent>
                ...
                <CardContent>
                  <ul className="space-y-2">
                    <li className="flex items-center gap-2">
                      ...
                      <span>Receive personalized connection suggestions for job leads</span>
                    </li>
                    <li className="flex items-center gap-2">
                      <div className="h-6 w-6 rounded-full bg-green-100 flex items-center justify-center text-green-600">✓</div>
                      <span>Automatically fill in job applications with your profile data</span>
                    </li>
                    <li className="flex items-center gap-2">
                      <div className="h-6 w-6 rounded-full bg-green-100 flex items-center justify-center text-green-600">✓</div>
                      <span>Get notified about network activities related to your job search</span>
                    </li>
                    <li className="flex items-center gap-2">
                      <div className="h-6 w-6 rounded-full bg-green-100 flex items-center justify-center text-green-600">✓</div>
                      <span>Import skills and experience to enhance your resumes</span>
                    </li>
                  </ul>
                </CardContent>
              </Card>
