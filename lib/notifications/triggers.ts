...
import { JobLead, JobLeadStatus } from "@prisma/client";
import { 
...
export async function triggerJobLeadStatusNotification(
  ...
  try {
    ...
    const lowPriorityTransitions = [
      // From ADDED to DISMISSED (user decided not to pursue)
      { from: JobLeadStatus.ADDED, to: JobLeadStatus.DISMISSED },
      // From DISMISSED to REJECTED (cleaning up)
      { from: JobLeadStatus.DISMISSED, to: JobLeadStatus.REJECTED },
    ];
...
export async function triggerOfferReceivedNotification(
  ...
  try {
    ...
    const notificationMetadata: ExtendedApplicationStatusMetadata = {
      ...
      newStatus: JobLeadStatus.OFFER_MADE,
      applicationId: leadId
    };
