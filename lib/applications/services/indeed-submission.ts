...
export async function submitIndeedApplication({
  ...
  coverLetterId,
  additionalInfo,
}: IndeedSubmissionOptions): Promise<{ success: boolean; message: string; applicationId?: string }> {
  // Get current user
  ...
  const applyUrl = jobLead.jobListing?.externalUrl || 
                   jobLead.url ||
  ...
  try {
    ...
    // Get resume data from the latest revision
    const resumeData = resume.latestRevision?.data as unknown as ResumeData;
    
    // Handle the Indeed application form
    await handleIndeedApplicationForm(page, resumeData, coverLetter?.content, additionalInfo);
...
async function handleIndeedApplicationForm(
  ...
  const isIndeedApplyPage = await page.evaluate(() => {
    return window.location.href.includes("indeed.com") && 
           (document.querySelector("form") !== null ||
            document.querySelector('[data-gnav-element-name="DisplayApply"]') !== null);
  });
  ...
  await handleResumeUpload(page, resumeData);
 ...
 */
async function fillPersonalInfo(page: Page, resumeData: ResumeData): Promise<void> {
  await page.evaluate((data) => {
    ...
    // Fill name fields
    fillField(['input[name*="first"][name*="name"]', 'input[placeholder*="First"]', 'input[id*="first_name"]'], 
              data.basics?.name?.split(' ')[0] || '');
    
    fillField(['input[name*="last"][name*="name"]', 'input[placeholder*="Last"]', 'input[id*="last_name"]'], 
    ...
    if (data.basics?.location) {
      fillField(['input[name*="city"]', 'input[id*="city"]'], 
                data.basics.location.city || '');
      
      fillField(['input[name*="state"]', 'input[id*="state"]', 'select[id*="state"]'], 
                data.basics.location.region || '');
      
      fillField(['input[name*="zip"]', 'input[name*="postal"]', 'input[id*="zip"]'], 
 ...
 */
async function handleResumeUpload(page: Page, resumeData: ResumeData): Promise<void> {
  // Check for resume upload field
  const resumeUploadExists = await page.evaluate(() => {
    return document.querySelector('input[type="file"]') !== null;
  });

  if (resumeUploadExists) {
    // This would be handled differently in a real implementation
    // as it would require generating and uploading an actual file
    console.log("Resume upload field detected but not implemented");
    
    // In a real implementation, we would generate a PDF from resumeData
    // and upload it using page.uploadFile()
  }
}
 ...
 */
async function fillCoverLetter(page: Page, coverLetterContent: string): Promise<void> {
...
async function handleScreeningQuestions(
  ...
  const hasScreeningQuestions = await page.evaluate(() => {
    ...
    const questionLabels = document.querySelectorAll('label, div.question, h3:not(:empty)');
  ...
  if (hasScreeningQuestions && additionalInfo) {
    ...
    await page.evaluate((info) => {
      ...
      Object.entries(info).forEach(([key, value]) => {
        ...
        const questionElements = Array.from(document.querySelectorAll('label, div.question, h3:not(:empty)'));
        
        for (const question of questionElements) {
          const questionText = question.textContent?.toLowerCase() || '';
          const keyLower = key.toLowerCase();
          
          if (questionText.includes(keyLower) || keyLower.includes(questionText)) {
