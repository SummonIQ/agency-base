...
const ATS_PATTERNS: Record<string, {
  urlPatterns: string[];
  domSelectors: string[];
  ...
  workday: {
    ...
    submitSelectors: ['[data-automation-id="submitApplication"]', '.workday-submit'],
    formPatterns: {
...
export class CompanyDirectSubmissionService implements SubmissionProvider {
  async submitApplication(params: SubmissionParams): Promise<SubmissionResult> {
    try {
      ...
      if (!jobId || !jobUrl) {
        throw new Error('Job ID and URL are required for company direct submissions');
      }
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Unknown error during company application',
        metadata: {
          provider: 'COMPANY_DIRECT',
      ...
      for (const pattern of patterns.urlPatterns) {
        if (url.includes(pattern)) {
          return {
            ...
            successIndicators: ['Application submitted successfully', 'Thank you for applying'],
          };
        }
      }
    }
  ...
  private async prepareFormData(
    ...
    return {
      ...
      files: {
        ...
        coverLetterFile: coverLetterData instanceof Buffer ? coverLetterData : undefined,
      },
  ...
  private async submitToCompanyDirect(
    ...
    try {
      ...
      switch (atsDetection.provider) {
        case 'workday':
          return await this.handleWorkdayApplication(jobData, formData, atsDetection);
        case 'greenhouse':
          return await this.handleGreenhouseApplication(jobData, formData, atsDetection);
        case 'lever':
          return await this.handleLeverApplication(jobData, formData, atsDetection);
        case 'bamboohr':
          return await this.handleBambooHRApplication(jobData, formData, atsDetection);
        case 'jobvite':
          return await this.handleJobviteApplication(jobData, formData, atsDetection);
        case 'smartrecruiters':
          return await this.handleSmartRecruitersApplication(jobData, formData, atsDetection);
        default:
          return await this.handleGenericApplication(jobData, formData, atsDetection);
      }
    } catch (error) {
      return {
        success: false,
  ...
  private async handleGenericApplication(
    ...
    return {
      success: false,
      error: 'Unknown ATS system detected. Application added to manual review queue for custom handling.',
  ...
  static isCompanyDirectJob(url: string): boolean {
    ...
    const jobBoards = [
      ...
      'wellfound.com', 'angel.co', 'monster.com', 'careerbuilder.com'
    ];
  ...
  async getATSCompatibility(url: string): Promise<{
    ...
    if (detection.provider === 'unknown') {
      recommendations.push('Consider applying manually for better success rate');
    }
