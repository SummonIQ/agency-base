...
export interface UnifiedApplicationParams {
  ...
  platformOverride?: JobBoard; // Force specific platform if detection fails
}
...
export class UnifiedApplicationService {
  ...
  static async submitApplication(
    ...
    try {
      ...
      return {
        success: false,
        platform: JobBoard.OTHER,
        platformDetection: { platform: JobBoard.OTHER, confidence: 0, indicators: [] },
  ...
  static async getApplicationStatus(userId: string, limit = 50): Promise<Array<{
    ...
    return applications.map(app => ({
      ...
      platform: app.jobLead.jobListing?.jobBoard || JobBoard.OTHER,
      status: app.status,
   ...
   */
  static async getPlatformStatistics(userId: string): Promise<Record<JobBoard, {
    ...
    return stats as Record<JobBoard, any>;
  }
