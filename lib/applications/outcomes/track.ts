import { ApplicationStatus, type ApplicationSubmission, type Prisma } from '@prisma/client';
import { db } from '@/lib/db';
import { revalidateTag } from '@/lib/cache';
import { sendDataUpdate } from '@/lib/events';
...
export async function trackApplicationOutcome(
  ...
  const daysSinceSubmission = submission.submittedAt
    ? Math.floor((now.getTime() - submission.submittedAt.getTime()) / (1000 * 60 * 60 * 24))
    : null;

  // Determine if this is the first response
  const isFirstResponse = !submission.responseReceivedAt && [
  ...
  const interviewCount = outcome.status === ApplicationStatus.INTERVIEW_SCHEDULED
 ...
 */
function getEventType(previousStatus: ApplicationStatus, newStatus: ApplicationStatus): string {
  if (newStatus === ApplicationStatus.UNDER_REVIEW) {
    return 'response_received';
  }
  if (newStatus === ApplicationStatus.INTERVIEW_REQUESTED || 
  ...
  if (newStatus === ApplicationStatus.NOT_SELECTED || 
 ...
 */
export async function getApplicationOutcomeHistory(
  submissionId: string
) {
