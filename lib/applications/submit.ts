import { ApplicationStatus, type Prisma, type Resume, type CoverLetter } from '@prisma/client';
...
import { submitToJobBoard } from './services/index';
...
async function checkRateLimits(userId: string): Promise<{
  ...
  const minIntervalAgo = new Date(now.getTime() - RATE_LIMITS.MIN_INTERVAL_MINUTES * 60 * 1000);
  ...
  if (lastSubmission && lastSubmission.createdAt > minIntervalAgo) {
    return {
      canSubmit: false,
      nextAvailableTime: new Date(lastSubmission.createdAt.getTime() + RATE_LIMITS.MIN_INTERVAL_MINUTES * 60 * 1000),
...
async function updateApplicationStatus(
  ...
  if (submission) {
    ...
    if (previousStatus && previousStatus !== status && submission.jobLead) {
      const jobTitle = submission.jobLead.jobListing?.title || submission.jobLead.title || 'Unknown Position';
      const companyName = submission.jobLead.jobListing?.company || 'the company';

      await createApplicationStatusNotification(submission.userId, {
        jobLeadId: submission.jobLeadId,
        jobTitle,
        companyName,
        previousStatus,
        newStatus: status,
...
export async function submitApplication(
  ...
  try {
    ...
    if (options.resumeId) {
      ...
      // Check for resume URL (either from revisions or main resume)
      const resumeUrl = resume?.revisions?.[0]?.pdfDocumentUrl || resume?.url;
    ...
    const submissionResult = await submitToJobBoard(
      jobLead.jobListing.jobBoard,
      {
        ...
        jobUrl: jobLead.jobListing.applyUrl || jobLead.jobListing.externalUrl || '',
        resumeData,
        coverLetterData,
        customFields: {
    ...
    await updateApplicationStatus(
      submissionId,
      submissionResult.success ? ApplicationStatus.SUBMITTED : ApplicationStatus.FAILED,
