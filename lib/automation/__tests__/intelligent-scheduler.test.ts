...
describe('IntelligentScheduler', () => {
  ...
  describe('scheduleApplications', () => {
    it('should schedule applications with intelligent timing', async () => {
      ...
      (db.jobLead.findMany as jest.Mock).mockResolvedValue(mockJobLeads);
      (db.automationScheduledApplication.findMany as jest.Mock).mockResolvedValue([]);
    ...
    it('should prioritize newer job postings when enabled', async () => {
      ...
      (db.jobLead.findMany as jest.Mock).mockResolvedValue([oldJob, newJob]);
      (db.automationScheduledApplication.findMany as jest.Mock).mockResolvedValue([]);

      const result = await scheduler.scheduleApplications(['old-job', 'new-job']);
      ...
      expect(newJobSchedule!.priority).toBeGreaterThan(oldJobSchedule!.priority);
    ...
    it('should respect rate limits when scheduling', async () => {
      ...
      (db.jobLead.findMany as jest.Mock).mockResolvedValue(manyJobs);
      (db.automationScheduledApplication.findMany as jest.Mock).mockResolvedValue([]);
      ...
      const sortedByTime = result.sort((a, b) => 
        a.scheduledFor.getTime() - b.scheduledFor.getTime()
      );

      for (let i = 1; i < sortedByTime.length; i++) {
        const timeDiff = sortedByTime[i].scheduledFor.getTime() - 
    ...
    it('should handle platform-specific optimal windows', async () => {
      ...
      (db.jobLead.findMany as jest.Mock).mockResolvedValue([linkedInJob]);
      (db.automationScheduledApplication.findMany as jest.Mock).mockResolvedValue([]);
  ...
  describe('rescheduleFailedApplication', () => {
    it('should reschedule failed application with exponential backoff', async () => {
      ...
      (db.automationScheduledApplication.findUnique as jest.Mock).mockResolvedValue(failedApp);
      (db.automationScheduledApplication.update as jest.Mock).mockResolvedValue({
        ...failedApp,
        status: 'scheduled',
      });
      (db.automationScheduledApplication.findMany as jest.Mock).mockResolvedValue([]);
    ...
    it('should not reschedule if max attempts reached', async () => {
      ...
      const result = await scheduler.rescheduleFailedApplication('failed-app-id');
  ...
  describe('Edge cases and business hours', () => {
    it('should skip weekends when weekdaysOnly is enabled', async () => {
      ...
      (db.jobLead.findMany as jest.Mock).mockResolvedValue([job]);
      (db.automationScheduledApplication.findMany as jest.Mock).mockResolvedValue([]);
    ...
    it('should respect business hours when enabled', async () => {
      ...
      const result = await scheduler.scheduleApplications(['business-hours-test']);
