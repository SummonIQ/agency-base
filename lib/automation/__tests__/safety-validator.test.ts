...
describe('SafetyValidator', () => {
  ...
  describe('Critical Safety Controls', () => {
    ...
    it('should block duplicate applications', async () => {
      mockDb.applicationSubmission.findFirst.mockResolvedValue({ id: 'existing-app' });
  ...
  describe('Company Blacklist', () => {
    ...
    it('should block applications with partial company name matches', async () => {
      const partialMatchJob = { ...mockJobLead, companyName: 'BadCompany Inc Solutions' };
      const validator = new SafetyValidator(defaultSettings, mockUserId);
  ...
  describe('Keyword Blacklist', () => {
    it('should block jobs with blacklisted keywords in title', async () => {
      const keywordJob = { ...mockJobLead, title: 'Unpaid Software Engineer Internship' };
      const validator = new SafetyValidator(defaultSettings, mockUserId);
  ...
  describe('Salary Threshold', () => {
    it('should block jobs below salary threshold', async () => {
      const lowSalaryJob = { ...mockJobLead, salaryMin: 30000, salaryMax: 40000 };
      const validator = new SafetyValidator(defaultSettings, mockUserId);
    ...
    it('should allow jobs above salary threshold', async () => {
      const goodSalaryJob = { ...mockJobLead, salaryMin: 60000, salaryMax: 80000 };
      const validator = new SafetyValidator(defaultSettings, mockUserId);
  ...
  describe('Company Application Limits', () => {
    it('should block when max applications per company is reached', async () => {
      ...
      expect(result.isAllowed).toBe(false);
      expect(result.reason).toContain('Maximum applications per company reached');
  ...
  describe('Consecutive Failures', () => {
    it('should block when too many consecutive failures', async () => {
      ...
      mockDb.applicationSubmission.findMany.mockResolvedValue(failedApplications);
    ...
    it('should allow when recent applications include successes', async () => {
      ...
      mockDb.applicationSubmission.findMany.mockResolvedValue(mixedApplications);
  ...
  describe('Settings Validation', () => {
    it('should prevent disabling user approval while automation is active', () => {
      const currentSettings = { ...defaultSettings, isEnabled: true, isPaused: false };
      const newSettings = { requireUserApproval: false };

      const result = SafetyValidator.validateSettingsChange(currentSettings, newSettings);

      expect(result.allowed).toBe(false);
      expect(result.reason).toContain('Cannot disable user approval while automation is active');
    });
    ...
    it('should prevent disabling duplicate prevention while automation is active', () => {
      ...
      expect(result.allowed).toBe(false);
      expect(result.reason).toContain('Cannot disable duplicate prevention while automation is active');
    });

    it('should prevent setting unreasonable rate limits', () => {
      const currentSettings = defaultSettings;
      
      const hourlyResult = SafetyValidator.validateSettingsChange(currentSettings, { applicationsPerHour: 100 });
      expect(hourlyResult.allowed).toBe(false);
      expect(hourlyResult.reason).toContain('Hourly application limit cannot exceed 50');

      const dailyResult = SafetyValidator.validateSettingsChange(currentSettings, { applicationsPerDay: 500 });
      expect(dailyResult.allowed).toBe(false);
      expect(dailyResult.reason).toContain('Daily application limit cannot exceed 200');
    });

    it('should allow settings changes when automation is paused', () => {
      const currentSettings = { ...defaultSettings, isEnabled: true, isPaused: true };
      const newSettings = { requireUserApproval: false };
  ...
  describe('Comprehensive Safety Validation', () => {
    ...
    it('should handle jobs with missing salary information gracefully', async () => {
      const noSalaryJob = { ...mockJobLead, salaryMin: undefined, salaryMax: undefined };
      const validator = new SafetyValidator(defaultSettings, mockUserId);
