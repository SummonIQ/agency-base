...
import { addHours, addMinutes, isWeekend, isWithinInterval, setHours, startOfHour } from 'date-fns';
...
export class AutomationScheduler {
   ...
   */
  async scheduleApplications(jobLeadIds: string[]): Promise<ScheduledApplication[]> {
  ...
  private prioritizeJobLeads(leads: any[]): any[] {
    return leads.sort((a, b) => {
      ...
      if (this.settings.prioritizeNewListings) {
        const ageA = new Date().getTime() - new Date(a.jobListing.postedAt || a.createdAt).getTime();
        const ageB = new Date().getTime() - new Date(b.jobListing.postedAt || b.createdAt).getTime();
      ...
      if (this.isValidSlot(zonedTime)) {
        ...
        if (dayCount < perDay && hourCount < perHour) {
          ...
          if (hourCount >= perHour) {
            ...
            currentTime = setHours(currentTime, this.settings.preferredStartHour);
          }
        }
      } else {
  ...
  private getNextValidSlot(currentTime: Date, timezone: string): Date {
    ...
    if (this.settings.scheduleBusinessHoursOnly) {
      if (hour >= this.settings.preferredEndHour) {
        ...
        nextTime = addHours(nextTime, 24 - hour + this.settings.preferredStartHour);
   ...
   */
  async rescheduleFailedApplication(applicationId: string): Promise<Date | null> {
 ...
 */
export async function processScheduledApplications(): Promise<void> {
  ...
  for (const app of dueApplications) {
    try {
      ...
      // TODO: Trigger actual application submission
      // This would integrate with existing application submission services
      console.log(`Processing scheduled application ${app.id} for job ${app.jobLead.jobListing.title}`);

      // Mark as completed (temporarily - real implementation would check submission result)
      await db.automationScheduledApplication.update({
        where: { id: app.id },
        data: {
          status: 'completed',
      ...
      console.error(`Failed to process scheduled application ${app.id}:`, error);
      ...
      const scheduler = new AutomationScheduler(app.userId, app.user.automationSettings!);
