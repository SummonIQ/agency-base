...
import { LeadDataProvider, DataQualityScore, LeadIntelligenceStatus, LeadIntelligenceCompanySize, IntentDataSignal } from "@prisma/client";
...
async function trackApiUsage(
  ...
  await db.leadIntelligenceApiUsageTracking.upsert({
    where: {
 ...
 */
function mapCompanySize(employeeCountRange?: string): LeadIntelligenceCompanySize | null {
  if (!employeeCountRange) return null;

  const sizeMap: Record<string, LeadIntelligenceCompanySize> = {
    "1-10": LeadIntelligenceCompanySize.STARTUP,
    "11-50": LeadIntelligenceCompanySize.SMALL_BUSINESS,
    "51-200": LeadIntelligenceCompanySize.MID_MARKET,
    "201-1000": LeadIntelligenceCompanySize.ENTERPRISE,
    "1000+": LeadIntelligenceCompanySize.LARGE_ENTERPRISE,
    "1001-5000": LeadIntelligenceCompanySize.LARGE_ENTERPRISE,
    "5000+": LeadIntelligenceCompanySize.LARGE_ENTERPRISE,
  };

  return sizeMap[employeeCountRange] || LeadIntelligenceCompanySize.MID_MARKET;
}
...
export async function saveEnrichedLead(
  ...
  const company = await db.leadIntelligenceCompany.upsert({
  ...
  const enrichedLead = await db.enrichedLead.upsert({
    ...
    create: {
      ...
      dataProvider: LeadDataProvider.ZOOMINFO,
      dataQuality: calculateDataQuality(zoomInfoPerson),
      status: LeadIntelligenceStatus.NEW,
      lastEnrichedAt: new Date(),
      assignedToUserId: user.id,
    },
  });
