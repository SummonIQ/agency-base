...
import { LeadDataProvider, DataQualityScore, LeadIntelligenceStatus } from "@prisma/client";
import * as apolloClient from "./api/apollo-client";
import * as zoomInfoClient from "./api/zoominfo-client";
...
export async function queueLeadForEnrichment(
  ...
  await db.leadIntelligenceEnrichmentQueue.create({
    data: {
      searchResultId,
      priority,
      status: "QUEUED",
...
export async function processLeadEnrichment(queueId: string): Promise<EnrichmentResult> {
  const queueItem = await db.leadIntelligenceEnrichmentQueue.findUnique({
    where: { id: queueId },
    include: {
      searchResult: true,
    },
  ...
  await db.leadIntelligenceEnrichmentQueue.update({
    where: { id: queueId },
    data: {
      status: "PROCESSING",
      startedAt: new Date(),
  ...
  try {
    ...
    if (result.leadId) {
      await db.leadIntelligenceSearchResult.update({
...
export async function searchProspects(request: SearchRequest): Promise<SearchResult> {
  ...
  const leadSearch = await db.leadIntelligenceSearch.create({
    data: {
      userId: user.id,
  ...
  try {
    ...
    for (const person of apolloResults.people) {
      try {
        const searchResult = await db.leadIntelligenceSearchResult.create({
          data: {
  ...
  try {
    ...
    for (const person of zoomInfoResults.data) {
      try {
        ...
        const existingResult = await db.leadIntelligenceSearchResult.findFirst({
          where: {
            leadSearchId: leadSearch.id,
            email: person.email,
          },
  ...
  await db.leadIntelligenceSearch.update({
...
export async function processEnrichmentQueue(batchSize: number = 10): Promise<{
  ...
  const queueItems = await db.leadIntelligenceEnrichmentQueue.findMany({
    where: {
      status: "QUEUED",
      OR: [
...
export async function getEnrichmentQueueStatus(): Promise<{
  ...
  const [queued, processing, completed, failed, retrying] = await Promise.all([
    db.leadIntelligenceEnrichmentQueue.count({ where: { status: "QUEUED" } }),
    db.leadIntelligenceEnrichmentQueue.count({ where: { status: "PROCESSING" } }),
    db.leadIntelligenceEnrichmentQueue.count({ where: { status: "COMPLETED" } }),
    db.leadIntelligenceEnrichmentQueue.count({ where: { status: "FAILED" } }),
    db.leadIntelligenceEnrichmentQueue.count({ where: { status: "RETRYING" } }),
  ]);
...
export async function retryFailedEnrichments(maxRetries: number = 3): Promise<number> {
  ...
  const failedItems = await db.leadIntelligenceEnrichmentQueue.findMany({
    where: {
      status: "FAILED",
  ...
  await db.leadIntelligenceEnrichmentQueue.updateMany({
    where: {
