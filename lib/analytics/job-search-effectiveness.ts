...
import { JobBoard, ApplicationStatus } from '@prisma/client';
import { 
  startOfWeek, 
  startOfMonth, 
  endOfWeek, 
...
export interface SearchStrategyMetrics {
  ...
  topKeywords: Array<{ keyword: string; frequency: number; successRate: number }>;
  platforms: Array<{ platform: JobBoard; usage: number; successRate: number }>;
...
export interface ApplicationTimingMetrics {
  ...
  platforms: Record<JobBoard, { applications: number; successRate: number }>;
...
export interface KeywordPerformanceMetrics {
  ...
  successfulApplications: number;
  successRate: number;
  platforms: Record<JobBoard, { searches: number; jobsFound: number }>;
  relatedKeywords: string[];
  optimization: 'expand' | 'refine' | 'maintain' | 'replace';
}
...
export class JobSearchEffectivenessAnalyzer {
  ...
  static async analyzeSearchStrategies(
    ...
    for (const [strategy, strategySearches] of strategyGroups) {
      ...
      const averageJobsPerSearch = totalSearches > 0 ? totalJobsFound / totalSearches : 0;
      ...
      const averageTimeToApplication = timeToApplications.length > 0
        ? timeToApplications.reduce((sum, time) => sum + time, 0) / timeToApplications.length
        : 0;

      // Extract keywords and their performance
      const keywordMap = new Map<string, { frequency: number; applications: number; successes: number }>();
      
      for (const search of strategySearches) {
        const keywords = this.extractKeywords(search.query || search.jobTitle || '');
        for (const keyword of keywords) {
          if (!keywordMap.has(keyword)) {
            keywordMap.set(keyword, { frequency: 0, applications: 0, successes: 0 });
          }
          ...
          const relatedJobLeads = search.jobLeads.filter(j => 
            (j.jobTitle + ' ' + (j.description || '')).toLowerCase().includes(keyword.toLowerCase())
          );
          const relatedApplications = relatedJobLeads.flatMap(j => j.applicationSubmissions);
      ...
      const platformMap = new Map<JobBoard, { usage: number; applications: number; successes: number }>();
      
      for (const jobLead of allJobLeads) {
        const platform = jobLead.jobListing?.jobBoard || JobBoard.OTHER;
        if (!platformMap.has(platform)) {
          platformMap.set(platform, { usage: 0, applications: 0, successes: 0 });
        }
  ...
  static async analyzeApplicationTiming(
    ...
    for (const app of applications) {
      ...
      const daySlot = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'][dayOfWeek];
    ...
    for (const [timeSlot, slotApplications] of timeSlots) {
      ...
      const responseTimes = slotApplications
        .filter(a => a.submittedAt && a.updatedAt && a.status !== ApplicationStatus.PENDING)
      ...
      // Platform breakdown
      const platformStats: Record<string, { applications: number; successes: number }> = {};
      
      for (const app of slotApplications) {
        const platform = app.jobLead.jobListing?.jobBoard || JobBoard.OTHER;
      ...
      const platforms = Object.fromEntries(
        Object.entries(platformStats).map(([platform, stats]) => [
          platform as JobBoard,
          {
            applications: stats.applications,
            successRate: stats.applications > 0 ? (stats.successes / stats.applications) * 100 : 0,
          },
        ])
      ) as Record<JobBoard, { applications: number; successRate: number }>;
      ...
      if (totalApplications >= 5) { // Only include slots with meaningful data
        timingMetrics.push({
          timeSlot,
          totalApplications,
          successfulApplications,
  ...
  static async analyzePlatformROI(
    ...
    const platformStats = new Map<JobBoard, {
      searches: Set<string>;
      jobsFound: number;
      applications: number;
      successes: number;
    ...
    for (const jobLead of jobLeads) {
      ...
      stats.totalTimeInvested += jobLead.applicationSubmissions.length * timePerApplication;
    ...
    for (const [platform, stats] of platformStats) {
      ...
      const roi = averageTimeInvested > 0 ? successRate / averageTimeInvested : 0;
      const qualityScore = stats.qualityScores.length > 0
  ...
  static async analyzeKeywordPerformance(
    ...
    const keywordMap = new Map<string, {
      ...
      applications: number;
      successes: number;
      platforms: Map<JobBoard, { searches: number; jobsFound: number }>;
    }>();
    ...
    for (const search of jobSearches) {
      ...
      for (const keyword of keywords) {
        ...
        const relevantJobs = search.jobLeads.filter(job =>
          (job.jobTitle + ' ' + (job.description || '')).toLowerCase().includes(keyword.toLowerCase())
        );
        stats.relevantJobsFound += relevantJobs.length;

        // Count applications and successes for relevant jobs
        const applications = relevantJobs.flatMap(job => job.applicationSubmissions);
        ...
        for (const job of search.jobLeads) {
          const platform = job.jobListing?.jobBoard || JobBoard.OTHER;
    ...
    for (const [keyword, stats] of keywordMap) {
      ...
      const relevanceScore = stats.totalJobsFound > 0 ? (stats.relevantJobsFound / stats.totalJobsFound) * 100 : 0;
      const successRate = stats.applications > 0 ? (stats.successes / stats.applications) * 100 : 0;

      const platforms = Object.fromEntries(
        Array.from(stats.platforms.entries()).map(([platform, platformStats]) => [
          platform,
          platformStats,
        ])
      ) as Record<JobBoard, { searches: number; jobsFound: number }>;

      // Determine optimization recommendation
      let optimization: 'expand' | 'refine' | 'maintain' | 'replace' = 'maintain';
      ...
      const relatedKeywords = this.findRelatedKeywords(keyword, Array.from(keywordMap.keys()));
  ...
  static async generateEffectivenessSummary(
    ...
    const previousDateRange = {
      start: period === 'week' ? startOfWeek(subWeeks(now, 1)) : startOfMonth(subMonths(now, 1)),
      end: period === 'week' ? endOfWeek(subWeeks(now, 1)) : endOfMonth(subMonths(now, 1)),
    };
    ...
    const previousOverview = await this.calculateOverviewMetrics(userId, previousDateRange);

    // Generate recommendations
    const recommendations = this.generateRecommendations(strategies, timings, platforms, keywords);

    // Calculate improvements
    const improvements = {
      comparedToPrevious: {
        successRateChange: overview.overallSuccessRate - (previousOverview.overallSuccessRate || 0),
        efficiencyChange: overview.efficiency - (previousOverview.efficiency || 0),
        volumeChange: overview.totalApplications - (previousOverview.totalApplications || 0),
      },
    ...
    return {
      ...
      bestTimings: timings.filter(t => t.recommendation === 'optimal').slice(0, 5),
  ...
  private static categorizeSearchStrategy(search: any): string {
    ...
    if (keywords.length <= 3) return `focused search: ${keywords.slice(0, 2).join(', ')}`;
    return 'broad keyword search';
  }

  private static extractKeywords(text: string): string[] {
    // Simple keyword extraction - can be enhanced with NLP
    const stopWords = new Set(['and', 'or', 'the', 'in', 'at', 'for', 'with', 'by', 'as', 'is']);
    return text
      .toLowerCase()
      .split(/[\s,.-]+/)
  ...
  private static findRelatedKeywords(keyword: string, allKeywords: string[]): string[] {
    // Simple similarity check - can be enhanced with semantic analysis
    return allKeywords
      .filter(k => k !== keyword && (k.includes(keyword) || keyword.includes(k)))
      .slice(0, 5);
  }
  ...
  private static async calculateOverviewMetrics(
    ...
    const overallSuccessRate = applications.length > 0 ? (successfulApplications / applications.length) * 100 : 0;
    const timeInvested = (searches * 30 + applications.length * 15) / 60; // Estimated hours
    const efficiency = timeInvested > 0 ? successfulApplications / timeInvested : 0;
  ...
  private static calculateTrends(
    ...
    const successRateChange = current.overallSuccessRate - (previous.overallSuccessRate || 0);
    trends.push({
      metric: 'Success Rate',
      direction: successRateChange > 1 ? 'up' : successRateChange < -1 ? 'down' : 'stable',
      percentage: Math.abs(successRateChange),
    });

    const volumeChange = ((current.totalApplications - (previous.totalApplications || 0)) / (previous.totalApplications || 1)) * 100;
    trends.push({
    ...
    trends.push({
      metric: 'Efficiency',
      direction: efficiencyChange > 0.1 ? 'up' : efficiencyChange < -0.1 ? 'down' : 'stable',
