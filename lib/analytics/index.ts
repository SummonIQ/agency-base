...
export interface JobBoardPerformanceData {
  jobBoard: JobBoard;
  applications: number;
  responses: number;
  interviews: number;
...
export interface AnalyticsData {
  ...
  jobBoardPerformance: JobBoardPerformanceData[];
...
export async function getUserAnalytics(
  ...
  const jobBoardPerformance = await calculateJobBoardPerformance(applications);
 ...
 */
async function calculateJobBoardPerformance(applications: any[]): Promise<JobBoardPerformanceData[]> {
  // Group applications by job board
  const jobBoardApplications = applications.reduce((acc, app) => {
    if (!app.jobLead?.jobListing?.jobBoard) return acc;
    
    const jobBoard = app.jobLead.jobListing.jobBoard;
    if (!acc[jobBoard]) {
      acc[jobBoard] = [];
    }
    acc[jobBoard].push(app);
    return acc;
  }, {} as Record<JobBoard, any[]>);
  
  // Calculate metrics for each job board
  return Object.entries(jobBoardApplications).map(([jobBoard, apps]) => {
