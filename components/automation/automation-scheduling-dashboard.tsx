...
export function AutomationSchedulingDashboard() {
  ...
  const [selectedApp, setSelectedApp] = useState<ScheduledApplication | null>(null);
  ...
  const fetchScheduledApplications = async () => {
    try {
      const response = await fetch('/api/automation/schedule?status=scheduled&limit=50');
      if (response.ok) {
  ...
  const handleReschedule = async () => {
    ...
    try {
      const response = await fetch(`/api/automation/schedule/${selectedApp.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ scheduledFor: newScheduledDate.toISOString() }),
      });
                        ...
                        {apps.map(app => (
                          ...
                          >
                            <div className="flex-1">
                              <h4 className="font-medium">{app.jobLead.jobListing.title}</h4>
                              <p className="text-sm text-muted-foreground">
                                {app.jobLead.jobListing.companyName}
                                {app.jobLead.jobListing.location && ` · ${app.jobLead.jobListing.location}`}
                              </p>
                            ...
                            <div className="flex items-center gap-2">
                              <Button
                                ...
                                onClick={() => {
                                  setSelectedApp(app);
                                  setNewScheduledDate(new Date(app.scheduledFor));
          ...
          <div className="space-y-4 py-4">
            {selectedApp && (
              <div className="p-3 bg-muted rounded-lg">
                <h4 className="font-medium">{selectedApp.jobLead.jobListing.title}</h4>
                <p className="text-sm text-muted-foreground">
