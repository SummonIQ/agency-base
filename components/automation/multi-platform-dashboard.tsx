...
const PLATFORM_ICONS: Record<string, React.ReactNode> = {
  ...
  GOOGLE: <Search className="h-5 w-5 text-red-600" />,
};
...
export function MultiPlatformDashboard() {
  ...
  const detectPlatform = async () => {
    ...
    try {
      const response = await fetch(`/api/automation/platforms?action=detect&jobUrl=${encodeURIComponent(urlInput)}`);
      const data = await response.json();
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">Loading platform information...</p>
        </div>
      </div>
    );
  }

  const implementedPlatforms = platforms.filter(p => p.implementationStatus === 'implemented');
  const plannedPlatforms = platforms.filter(p => p.implementationStatus === 'planned');

  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h2 className="text-3xl font-bold">Multi-Platform Application Integration</h2>
        <p className="text-muted-foreground mt-2">
          Automate job applications across {platforms.length} different platforms with intelligent detection and field mapping.
        </p>
      </div>
      ...
      <Tabs defaultValue="implemented" className="space-y-4">
        ...
        <TabsContent value="planned" className="space-y-4">
          <Alert>
            ...
            <AlertDescription>
              These platforms are planned for future implementation. They will be added to our automation capabilities in upcoming releases.
            </AlertDescription>
          </Alert>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-blue-600">
                  {implementedPlatforms.filter(p => p.capabilities.confidenceLevel === 'high').length}
                </div>
                <p className="text-xs text-muted-foreground">
                  ...
                  <div key={platform.platform} className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {platform.implementationStatus === 'implemented' && (
                        getConfidenceBadge(platform.capabilities.confidenceLevel)
                      )}
                    </div>
                  </div>
                ))}
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                ...
                {selectedPlatform.implementationStatus === 'implemented' && (
                  getConfidenceBadge(selectedPlatform.capabilities.confidenceLevel)
                )}
              </div>
              ...
              <div>
                ...
                <ul className="list-disc list-inside text-sm space-y-1">
                  {selectedPlatform.capabilities.features.map((feature, index) => (
              ...
              <div>
                ...
                <ul className="list-disc list-inside text-sm space-y-1 text-muted-foreground">
                  {selectedPlatform.capabilities.limitations.map((limitation, index) => (
      ...
      <CardContent>
        <div className="space-y-3">
          <div className="flex items-center justify-between text-sm">
            ...
            <Badge variant={platform.capabilities.automationSupported ? 'success' : 'secondary'}>
          ...
          {platform.capabilities.automationSupported && (
            <div className="flex items-center justify-between text-sm">
              ...
              <Badge className={cn(
                platform.capabilities.confidenceLevel === 'high' && 'bg-green-100 text-green-800',
                platform.capabilities.confidenceLevel === 'medium' && 'bg-yellow-100 text-yellow-800',
                platform.capabilities.confidenceLevel === 'low' && 'bg-red-100 text-red-800'
              )}>
          ...
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="text-muted-foreground">{platform.requiredFields.length}</span>
          </div>
