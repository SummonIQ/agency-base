...
export function AutomationAnalyticsDashboard() {
  ...
  const fetchMetrics = async () => {
    try {
      const response = await fetch(`/api/automation/analytics?range=${dateRange}`);
      if (response.ok) {
  ...
  const handleExport = async () => {
    try {
      const response = await fetch(`/api/automation/analytics/export?range=${dateRange}`);
      if (response.ok) {
  ...
  const automationRate = totalApplications > 0 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              ~{metrics.roiMetrics.averageTimePerManualApplication}min per manual app
            </p>
          </CardContent>
        </Card>
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={metrics.platformBreakdown}>
                ...
                <Tooltip />
                <Legend />
                <Bar yAxisId="left" dataKey="totalSubmissions" fill="#8884d8" name="Submissions" />
