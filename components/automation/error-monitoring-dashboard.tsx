...
export function ErrorMonitoringDashboard() {
  ...
  const fetchMetrics = async () => {
    try {
      ...
      const startDate = timeRange === '24h' ? subDays(endDate, 1) :
                       timeRange === '7d' ? subDays(endDate, 7) :
                       subDays(endDate, 30);
      ...
      logs.errors.forEach((error: any) => {
        if (error.platform) {
          platformErrors[error.platform] = (platformErrors[error.platform] || 0) + 1;
        }
      });
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Overall automation system health and error monitoring
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      {metrics.criticalErrors > 0 && (
        <Alert variant="destructive">
          ...
          <AlertDescription>
            {metrics.criticalErrors} critical error(s) require immediate attention.
            Review the error dashboard for details.
          </AlertDescription>
        </Alert>
      )}
        ...
        <Card>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={250}>
              <PieChart>
                <Pie
                  data={Object.entries(metrics.errorsByCategory).map(([name, value]) => ({
                    name,
                    value,
                  }))}
                  cx="50%"
                  ...
                  label
                >
                  {Object.keys(metrics.errorsByCategory).map((category, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={CATEGORY_COLORS[category as keyof typeof CATEGORY_COLORS] || '#8884d8'}
                    />
                  ))}
                </Pie>
                <Tooltip />
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={250}>
              ...
              >
                ...
                <Tooltip />
                <Bar dataKey="count">
                  {Object.keys(metrics.errorsBySeverity).map((severity, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={SEVERITY_COLORS[severity as keyof typeof SEVERITY_COLORS] || '#8884d8'}
                    />
                  ))}
                </Bar>
              </BarChart>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-3">
              {Object.entries(metrics.platformErrors).map(([platform, count]) => (
            ...
            <div className="space-y-4">
              <div>
                <div className="flex justify-between mb-1">
                  ...
                  <span className="text-sm font-medium">{metrics.resolutionRate.toFixed(1)}%</span>
                </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-3">
              ...
              <StatusIndicator
                label="Auto Recovery"
                status={metrics.resolutionRate > 50 ? 'operational' : 'degraded'}
