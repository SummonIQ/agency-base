...
const formSchema = z.object({
  ...
  jobType: z.enum(["fulltime", "parttime", "contract", "internship", "temporary", ""]).optional(),
...
export function IndeedSearchForm({ hasApiKey, recentSearches = [] }: IndeedSearchFormProps) {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        router.push(`/job-searches/${result.jobSearchId}`);
      } else {
        toast({
          variant: "destructive",
          title: "Search failed",
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        ...
        {!hasApiKey && (
          <div className="text-sm text-amber-500 bg-amber-50 p-3 rounded-md">
            <p>You haven't set up your Indeed API key yet. Search results may be limited.</p>
          </div>
        )}
