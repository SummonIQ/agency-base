...
export function JobSearchProgress({ jobSearchId, onComplete }: JobSearchProgressProps) {
    ...
    const fetchProgress = async () => {
      try {
        ...
        if (result) {
          ...
          // Count jobs found
          if (result._count?.jobListings) {
            setJobsFound(result._count.jobListings);
          }
          ...
          if (result.status === JobSearchStatus.COMPLETED) {
            ...
            toast({
              ...
              description: `Found ${result._count?.jobListings || 0} job listings that match your criteria.`,
              variant: "default",
            });
